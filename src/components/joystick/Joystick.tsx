...
import { JoystickShape } from "./enums/shape.enum.ts";
...
class Joystick extends React.Component<IJoystickProps, IJoystickState> {
	...
	private _lastPointerMoveEvent: PointerEvent;
	private _hasMoved = false;
	private _intervalMove;
	...
	private _pointerDown(e: PointerEvent) {
		...
		this._intervalMove = setInterval(() => {
			if (this.state.dragging && !this._pointerIsMoving && this._hasMoved) {
				this._pointerMove(this._lastPointerMoveEvent);
	...
	private _PointerDownNotMoving(e: PointerEvent) {}
	...
	private _pointerMove = (event: PointerEvent) => {
		this._hasMoved = true;
		if (event != this._lastPointerMoveEvent) {
	...
	private _pointerUp = (event: PointerEvent) => {
		...
		clearInterval(this._intervalMove);
		...
		if (this.props.stop) {
			this.props.stop({
				...
				// @ts-ignore
				x:
					this.props.sticky && this.state.coordinates
						? (this.state.coordinates.relativeX * 2) / this._baseSize
						: null,
				// @ts-ignore
				y:
					this.props.sticky && this.state.coordinates
						? (this.state.coordinates.relativeY * 2) / this._baseSize
						: null,
				// @ts-ignore
				direction:
					this.props.sticky && this.state.coordinates
						? this.state.coordinates.direction
						: null,
				// @ts-ignore
				distance:
					this.props.sticky && this.state.coordinates
						? this.state.coordinates.distance
						: null,
			});
		}
	};
	...
	private _getStickStyle(): any {
		...
		if (this.props.pos) {
			stickStyle = Object.assign({}, stickStyle, {
				position: "absolute",
				transform: `translate3d(${(this.props.pos.x * this._baseSize) / 2}px, ${
					-(this.props.pos.y * this._baseSize) / 2
				}px, 0)`,
			});
		}
		...
		return (
			<div
				...
				className={`${this.props.baseClassName} ${
					this.props.disabled ? "joystick-base-disabled" : ""
				}`}
				ref={this._baseRef}
				style={baseStyle}
			>
